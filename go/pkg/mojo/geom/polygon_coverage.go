...
// Notice: Holes in polygon are covered too!
func (m Polygon) CellCovering(minLevel, maxLevel, maxCells int) s2.CellUnion {
	rc := &s2.RegionCoverer{MinLevel: minLevel, MaxLevel: maxLevel, MaxCells: maxCells}

	var points []s2.Point
	for _, ll := range m.LineStrings[0].Coordinates {
...
// Notice: Holes in polygon are not covered!
func (m Polygon) CellCoveringExceptHoles(minLevel, maxLevel, maxCells int) s2.CellUnion {
	rc := &s2.RegionCoverer{MinLevel: minLevel, MaxLevel: maxLevel, MaxCells: maxCells}
	pr := newPolygonRegion(m)
	return rc.Covering(pr)
}

func (m Polygon) GeoHashCovering(minLevel, maxLevel, maxHashes int) []string {
	return m.GeoHashFractionCovering(minLevel, maxLevel, maxHashes, 0)
}

func (m Polygon) GeoHashFractionCovering(minLevel, maxLevel, maxHashes int, fraction float32) []string {
	cells := m.CellCovering(18, 18, 10000)
