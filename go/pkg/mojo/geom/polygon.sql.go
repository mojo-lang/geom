...
func (m *Polygon) Scan(value interface{}) error {
	...
	case []byte:
		...
		if err == nil {
			m.LineStrings = append(m.LineStrings, NewLineString(points...))
		}
		return err
	case string:
		points, err := parsePolygon(string(bs))
...
func (m Polygon) Value() (driver.Value, error) {
	...
	for i, point := range m.LineStrings[0].Coordinates {
		if i > 0 {
			buffer.WriteByte(',')
		}
...
func (m Polygon) GormDataType() string {
	return "polygon"
}
